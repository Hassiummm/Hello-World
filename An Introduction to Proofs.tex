...
\par
In this section, we begin to investigate sets, the most basic entities in mathematics. It is natural to ask the question: What is a set? There is no precise definition of sets. Intuitively, a \hdef{set} is a collection of objects that satisfy some property, and these objects are called \hdef{elements}.
\par
...
\begin{remark}
    This note is based on ZFC set theory. In this system, every object is a set and sets have inheritoires, so there are sets of sets.
\end{remark}
...
\begin{example}
    The set $S=\{\es,\{\es\},\{\{\es\}\}\}$ has three elements.
\end{example}
\begin{example}
    The set $\{2n\mid n\in\N\}$ is the set of all even numbers, where $\N$ is the set of natural numbers. The set $\Q=\{a/b\mid a,b\in\Z\ \text{and}\ b\ne 0\}$ is the set of rational numbers, where $\Z$ is the set of integers.
\end{example}
\begin{problem}
    How many elements are in the set $S=\{1,2,\{3,4\},\{1,2\}\}$?
\end{problem}
\begin{problem}
...
\end{proposition}
\begin{proof}
    Let $A$ be a set. For all $x\in A$, $x\in A$, so $A\sub A$. 
\end{proof}
\begin{proposition}
...
\end{proof}
\begin{problem}
    Let $a$ be any object, prove that $\{a\}=\{a,a\}$.
\end{problem}
\begin{problem}
    How many subsets does $S=\{1,2,3\}$ have? What about $X=\{1,2,3,4\}$?
\end{problem}
\begin{problem}
...
\begin{Axiom of Union}\index{Axiom of Union}
    For all set $X$, there exists a set $Y=\Un X$, the union
...
\end{proof}
Some texts assume the existence of some ``universal set'', denoted $U$, which has all objects as elements including itself, so we can define complements of any set $S$ as the set $U\sm S$. However, this assumption leads to a paradox. 
\begin{example}
    Consider the set $S$, defined as the set of all sets that are not members of themselves, that is, $S=\{X\mid X\notin X\}$. Does $S$ belong to $S$? This is known as \hdef{Russell's Paradox}.
\end{example}
\par
...
\end{example}
\begin{problem}
    Let $X=\{a,b,c\}$, how many elements are in $\ps(X)$? What if $X=\{a,b,c,d\}$?
\end{problem}
\begin{definition}
    Let $X$ and $Y$ be sets. The \hdef{Cartesian product} $X\times Y$ is the set of all ordered pairs $(a,b)$, where $a\in X$ and $b\in Y$.
\end{definition}
\par
Why the Cartesian product $X\times Y$ is a set? We could rewrite $X\times Y\sub\ps\ps(X\cup Y)$ as $\{z\in\ps(\ps(X\cup Y))\mid\exists x\in X,\ \exists y\in Y,\ z=(x,y)\}$.
\begin{problem}
    Prove that $X\times Y=\{z\in\ps(\ps(X\cup Y))\mid\exists x\in X,\ \exists y\in Y,\ z=(x,y)\}$.
\end{problem}
\begin{problem}
    Let $A$, $B$, and $C$ be sets. Prove the following propositions.
    \begin{enumerate}
...
\end{problem}
\begin{problem}
    Let $A$ and $B$ be sets. Prove that $\es\times A=\es$. Prove that if $A\times B=\es$, then either $A=\es$ or $B=\es$.
\end{problem}
\begin{remark}
    Let $X$ be a set, then we denote ${X}^{n}=X\times\cdots\times X$, the Cartesian product of $n$ numbers of $X$.
\end{remark}
\begin{definition}
    A \hdef{binary operation} $R$ is a set of ordered pairs. If $(x,y)\in R$, we write $xRy$. The \hdef{domain} of $R$ is the set $\dom(R)=\{u\mid\exists v(u,v)\in R\}$. The \hdef{range} of $R$ is the set $\ran(R)=\{v\mid\exists u(u,v)\in R\}$. 
\end{definition}
\begin{problem}
    Why the domain and range of a binary operation $R$ are sets? Prove that $\dom(R),\ran(R)\sub\Un\Un R$.
\end{problem}
\begin{definition}
...
\end{definition}
\begin{example}
    The set of real numbers $\R$ forms a linearly ordered set in the natural ordering.
\end{example}
\begin{proposition}
    Let $(X,\le)$ be a partially ordered set and let $Y\sub X$, then $Y$ is a partially ordered.
\end{proposition}
\begin{proof}
...
\end{problem}
\begin{problem}
    Let $X$ be a set. If $(\ps(X),\sub)$ is a linearly ordered set, prove that either $X=\es$ or $X=\{a\}$.
\end{problem}
\begin{definition}
...
\end{definition}
\begin{problem}
    We say a set is finite if it has $n$ elements, where $n\in\N$. Let $X$ be a partially ordered set such that every subset of $X$ has a supremum and a infimum. Prove that $X$ is a finite linearly ordered set.
\end{problem}
\par
