...
\usepackage{textcomp,tocloft,environ,setspace,geometry,enumerate,enumitem,blindtext,multicol,xcolor,fancyhdr,calligra,graphicx,wrapfig,pgfplots,mdframed,tabularx,lipsum,comment,csquotes,zref-user,imakeidx,etoolbox,xparse,pgffor}
\usepackage{chemfig}
...
\newcommand{\hmain}{%
    ...
    \fancyhead[R]{\fontfamily{pzc}\selectfont{{\printpage}}}    
    \fancyhead[C]{\htitle}             
...
% Initialize a counter for unique labels
\newcounter{cfdcounter}

% Store a list of cfd terms and labels
\newcommand{\cfdlist}{}

% Define the \cfd command to store the term and create a label
\newcommand{\cfd}[1]{%
  \refstepcounter{cfdcounter}% Increment the counter for each \cfd{}
  \label{cfd:\thecfdcounter}% Create a label with the counter
  \xdef\cfdlist{\cfdlist#1}% Append the term and label to the list
  {\fontfamily{pzc}\selectfont{#1}\fontfamily{cmr}\selectfont{}}
}

% Define the \hindex command to generate an enumerated list with hyperlinks
\newcommand{\hindex}{%
  \newsection{Index} % This creates the Index section
  \def\templist{\cfdlist} % Use the list of terms stored
    \foreach \term in \templist {%
      \splitter{\term}{|}% Split by delimiter (counter and term)
  \begin{enumerate}
    \item \hyperlink{cfd:\tempa}{\tempa} \\% Create the hyperlink for each item
  \end{enumerate}
    }
   % End the enumerate list
}

% Helper command to split the list by a delimiter
\newcommand{\splitter}[2]{%
  \def\tempa{#1}% First part (counter) goes in #1
  \def\tempb{#2}% Second part (term) goes in #2
}
