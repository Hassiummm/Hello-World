...
\begin{verbatim}
    ...
    \end{document}
\end{verbatim}
Here the ``hfauthor'' is the left part of the header. Also, feel free to use ``hstart'' command to include all three setup.
\begin{verbatim}
    \documentclass{article}
    \input{hassium.tex}
    \begin{document}
...
\section{Darkmode}
Darkmode commands change the background color to black and the text to white.
\begin{verbatim}
    \begin{document}
        \darkhsetup
        \darkhmain
...
\begin{multicols}{2}
    ...
    $\bs$can\hfill$\can$ \\
    $\bs$Can\hfill$\Can$ \\
    $\bs$cat$\{A\}$\hfill$\cat{A}$ \\
    $\bs$cata$\{A\}\{B\}$\hfill$\cata{A}{B}$ \\
    $\bs$catb$\{A\}\{B\}$\hfill$\catb{A}{B}$ \\
    $\bs$catop$\{A\}$\hfill$\catop{A}$
\end{multicols}
